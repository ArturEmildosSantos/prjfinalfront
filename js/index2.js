...
function tratastatus(res){
    if(res.status == 200){
        ...
        document.getElementById("msgErro").innerHTML = `<br>Senha inválida`;
    }else if(res.status == 404){
        document.getElementById("msgErro").innerHTML = `<br>Usuário não encontrado`;
    }else{
        document.getElementById("msgErro").innerHTML = `<br>Erro desconhecido`;
    }
}
